...
public class StartActivity extends Activity {
    ...
    private static int g_attVal=60;

    private static int g_medVal= 20;

    private static int runningattValue=0;

    private static int status=0;
    private static int play_status=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        transferButton = (Button)this.findViewById(R.id.transferButton);
        ...
        nskAlgoSdk.setOnAttAlgoIndexListener(new NskAlgoSdk.OnAttAlgoIndexListener() {
            ...
            public void onAttAlgoIndex(int value) {
                ...
                runningattValue=value;

                if (value>=g_attVal-10 && play_status==1)
                {

                    showToast("Brain waves matched",Toast.LENGTH_LONG);
                    Intent intent = new Intent(context,TransferActivity.class);
                    play_status=0;
                    startActivity(intent);
    ...
    private String test_path="/sdcard/neurosky/test";
    private String train_path="/sdcard/neurosky/train";

    private void startstopRecording() {



        //default path /sdcard/neurosky/year-month-dayhour-minute-second-RawPackageRecording.txt
        //train_path=Environment.getExternalStorageDirectory()+"/neurosky/train.txt";


        Log.e("TAG", "Setting train_path " + train_path);
        //FIXME Commented temporarily
        //tgStreamReader.setRecordStreamFilePath(train_path);
        ...
        else
        {
            ...
            */
            ...
            //Set to Global Attention ..Trained attention
             //Doesnt work for TextView like below .. Temp. Hack by creating runningattValue
            //g_attVal=Integer.parseInt(attValue.getText().toString());
             g_attVal=runningattValue;
            showToast("Trained attention level"+g_attVal,Toast.LENGTH_SHORT);

        }


    }
    private ArrayList<String> preparedList =new ArrayList<String>();
    private ArrayList<String> testList =new ArrayList<String>();


    private void prepareTrainList() {

        Log.d(TAG, "Train Data");
        int size = 512 * raw_data_sec_len;
        ...
        try {
            for (int i = 0; i < listOfFiles.length; i++) {
                fileIn = new FileInputStream(new File(train_path+"/"+ listOfFiles[i].getName()));
                ...
                while (lineCount < size) {
                    ...
                    byte[] decoded = Base64.decode(line.getBytes(), Base64.DEFAULT);

                    Log.d(TAG, lineCount + ":" + line);
                    preparedList.add(decoded.toString());
                    lineCount++;
                }
                Log.d(TAG, "lineCount=" + lineCount);
    ...
    private void prepareTestList()
    {
        ...
        try {
            for (int i = 0; i < listOfFiles.length; i++) {
                fileIn = new FileInputStream(new File(test_path+"/"+listOfFiles[i].getName()));
                isr = new InputStreamReader(fileIn);
                lineCount = 0;
                reader = new BufferedReader(isr);
                while (lineCount < size) {
    ...
    private void validateRecording(Context context) {

        //get the current attention level and compared trained global attention

       // int c_attVal=Integer.parseInt(attValue.getText().toString());

        if(play_status==0) {

            play_status = 1;
            validateButton.setText("Stop");
        }
        else {
            play_status = 0;
            validateButton.setText("Play");
        }

        /*if (c_attVal>=g_attVal-10)
        {
            showToast("Brain waves matched",Toast.LENGTH_LONG);
            Intent intent = new Intent(context,TransferActivity.class);
            startActivity(intent);
        ...
        //FIXME The below logic is never executed for now
        //default path /sdcard/neurosky/year-month-dayhour-minute-second-RawPackageRecording.txt


        /*
        tgStreamReader.setRecordStreamFilePath(test_path);
        if(status==0)
        {
        ...
        else
        {
            ...
            prepareTestList();
            double matchPercent=compareStreams();
            //compare the stored train raw data
            ...
            else
            {
                ...
                showToast("Brain waves Not Matched", Toast.LENGTH_LONG);
...
*/
    ...
    private double compareStreams() {

        short matchctr=0;
        for (Iterator it=testList.iterator();it.hasNext();)
        {
            String rawCode=(String)it.next();
            if (preparedList.contains(rawCode))
                matchctr++;

        }

        return Math.ceil(matchctr/preparedList.size());
